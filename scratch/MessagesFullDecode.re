...
let mapToSchemaState: Js.t('a) => newMessage =
  newMessageJs => {
    ...
    /* annotate with string so that react.element can render or else throws error saying type is message. */
    let message:string = newMessageJs##newMessage##message;
  ...
  <SubscriptionWithHandler request handler>
    ...{({response}) =>
      ...
      | Data(data) =>
        ...
        let messages = newMessagesRaw->Belt.Array.map(mapToMessage);
        ...
        |> Array.mapi((index, m) =>
             <div className=Styles.dexContainer>
               <div key={index->string_of_int} className=Styles.dex>
                 <h1 className=Styles.dexTitle> m.newMessage.message ->React.string </h1>
               </div>
             </div>
           )
        |> React.array;
